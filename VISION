...
| Custom entities | `tavu_lead`, `tavu_opportunity`, `tavu_case`, `tavu_proposal`, `tavu_knowledge_article`, `tavu_systemsettings`, plus master configuration tables (`tavu_customertierdefinition`, `tavu_casetype`, `tavu_sla`) |
...
- **`tavu_opportunity` uses Discovery-driven stages** (`Discovery` → `Proposal Drafted` → `Proposal Sent` → `Negotiation` → `Won` / `Lost`), not the traditional Quote-driven flow. This reflects how professional services sell scope, not SKUs.
...
- **AI-Assisted Forecasting & Capacity Planning** — addresses Pain #2 and Pain #5. Combines pipeline data with delivery commitments and team capacity to produce more reliable revenue and resource forecasts. Requires sufficient historical data to be useful, which is why it is positioned as a roadmap item rather than an initial module.
...
- Opportunity pipeline with discovery-driven stages (`tavu_opportunity`)
...
OpenTavu is open-source under the MIT license. There is no commercial license tier, no enterprise edition, and no paid support contract sold by the framework itself. Implementation services, consulting engagements, and custom development are offered separately by the author and (in time) by other consultants in the community.
...
- An Azure subscription with Azure OpenAI access (or willingness to provision one), or an alternative AI provider via OpenTavu's provider-agnostic interface
