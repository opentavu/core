...
| Forecast Category | tavu\_forecastcategory | Choice (Omitted / Pipeline / Best Case / Committed / Closed) | Optional | Auto-populated from the Sales Stage's forecast category, but **settable per opportunity by the seller** — so a specific deal can be committed or held back independently of its stage. **Omitted** means the deal is excluded from pipeline, coverage, and the forecast (HubSpot-style "do not forecast"). Same defaulting pattern as Probability. See Section 6.3bis. |
...
| Business Line | tavu\_businessline | Lookup → tavu\_businessline | Optional | Practice / business-line slice for per-practice forecasting. Defaults from `tavu_systemsettings.tavu_defaultbusinessline` on create. Single-practice firms use the seeded "General Practice". See the forecasting schema in `docs/forecasting-build-plan.md`. |
| Sales Target | tavu\_salestarget | Lookup → tavu\_salestarget | Optional | System-managed stamp (owner, period, business line) that enables the native attainment rollups on `tavu_salestarget`. Hidden from sellers. See `docs/forecasting-build-plan.md` §6. |
...
| Forecast Category | tavu\_forecastcategory | Choice | Required | Omitted / Pipeline / Best Case / Committed / Closed |
...
| :---- | :---- | :---- |
| **Omitted** | Not forecasted. Excluded from pipeline, coverage, and the forecast (e.g. Nurture, On hold). | — |
...
**Phase-1 schema (finalized, design; not built yet).** The deterministic goal/quota and attainment layer is fully specified in `docs/forecasting-build-plan.md` (v1.1) with a companion `docs/forecasting-test-checklist.md`. It adds `tavu_businessline` (a lean business-line configuration table), a `tavu_goaltype` table (goal/forecast type; Phase 1 builds Revenue only, activity templates are Phase 2), extends `tavu_salestarget` to four scopes (User / Team / Business Line / Company) with native rollups on the user scope and flow-based aggregation for the higher scopes (Dataverse forbids a rollup over a rollup), stamps each opportunity to its (owner, period, business-line) target via the existing pre-op opportunity plugin, and adds `tavu_reportingcache` as the pre-aggregated read surface for a PCF/ECharts dashboard. The target period is **Month or Quarter** only (Quarter default for PS); the annual figure is the derived sum of the year's buckets, not a settable period, and the weekly snapshot ("the photo") is a separate axis. The forecast category gains an **Omitted** value (stages excluded from the forecast, HubSpot-style). Coverage seed default 2.0, verified against the SPI 2026 benchmark (industry bid win rate 45.1%, high performers 56.5%).
...
| 2.3 | August 21, 2026 | Gustavo González Villani (revision with Claude) | **AI-Assisted Forecasting Phase-1 schema finalized (design; not built yet).** Full build plan in `docs/forecasting-build-plan.md` (v1.0) with `docs/forecasting-test-checklist.md`. (1) Resolved the two open decisions: **quota granularity = User + Team + Business Line + Company from day one**; **SPI 2026 figures verified** (industry bid win rate 45.1%, high performers 56.5%; average coverage 158%, high performers 224%), confirming the ~2x professional-services coverage seed. (2) Added **`tavu_businessline`** (lean business-line configuration table) and opportunity fields **`tavu_businessline`** and system-managed **`tavu_salestarget`** stamp (§6.3). (3) Specified the lean quota schema: `tavu_salesperiod`, `tavu_salestarget` (four scopes, native rollups on the user scope for Closed Won / Committed / Best Case / Open Pipeline, plus a Manager Adjusted Amount and deterministic calculated columns for attainment %, gap-to-goal, forecast, coverage), `tavu_forecastsnapshot` (weekly pacing history + pre-aggregation), and `tavu_reportingcache` (dashboard read surface). (4) Documented the **rollup-over-rollup limitation**: Team / Business Line / Company scopes aggregate via `Fl.Forecast.SnapshotWeekly` (+ optional `Fl.Forecast.CacheNightly`) into the cache/snapshot, not native rollups. (5) Extended the pre-op opportunity plugin with forecast-category defaulting (mirroring probability), business-line default, and the target stamp. (6) PCF/ECharts dashboard (attainment, gap, coverage gauge with over-selling warning, pacing, pipeline-by-category) reading the pre-aggregated tables; row-level + hierarchy security; no Power BI. (7) New `tavu_systemsettings` fields: `tavu_coveragetarget` (2.0), `tavu_forecastperiodtype` (Quarter), `tavu_defaultbusinessline`, `tavu_forecastsnapshotdow` (Friday). |

| 2.4 | August 21, 2026 | Gustavo González Villani (revision with Claude) | **Forecasting refinements after reviewing HubSpot forecast configuration (design).** (1) **Renamed Service Line to Business Line** (`tavu_businessline`), OpenTavu terminology. (2) Added the **Omitted** forecast category (§6.3, §6.3bis) for stages/deals excluded from pipeline, coverage, and the forecast (HubSpot-style "do not forecast"). (3) Added a **`tavu_goaltype`** config table (goal/forecast type: metric kind, source, date property, amount property) in the build plan; **Phase 1 builds Revenue only**, activity goal templates (Meetings Booked, Proposals Sent, Deals Created, Calls) are **Phase 2**. (4) Target period is **Month or Quarter** only (Quarter default for PS); the annual is a derived sum (not a settable period), and the weekly snapshot is a separate axis. A **quota-entry grid** Custom Page for low-friction target capture (columns per period, Total = annual). (5) Full detail and a Phase-2 roadmap in `docs/forecasting-build-plan.md` v1.1. |
