...
namespace Pl.Proposal.LifecycleTracker
{
	...
	///
	///   - Create: defaults tavu_version to "v1" when not supplied.
	///   - Update:
	...
	public class LifecycleTracker : PluginBase
	{
		...
		private void GuardStatusTransition(LocalPluginContext localContext, Entity target, Entity preImage)
		{
			...
			bool wasSent = oldStatus.Value == StatusSentToClient || oldStatus.Value == StatusAwaitingDecision;
		...
		private static bool IsLocked(Entity image)
		{
			...
			return status != null &&
				   (status.Value == StatusSentToClient || status.Value == StatusAwaitingDecision);
		}
	}
}
